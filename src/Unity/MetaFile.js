...
export const loadAnimations = async (id) => {
  ...
  const mappedSpritesheet = spritesheet.reduce((state, current) => ({...state, [current.name]: current}), {})
  const spriteTexture = PIXI.Texture.fromImage(spriteUrl)
  const result = {};
  await Promise.all(ANIMATION_TYPES.map(async (type) => {
    const frames = [];
